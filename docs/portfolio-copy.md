...
v0.1 版本验证了 demo-ready 闭环：教师上传 PPTX/PDF，确认课程信息，系统生成互动课件蓝图和媒体计划，渲染 slide-based `lesson.html`，运行质量检查，并导出可离线打开的 HTML ZIP 或可编辑课堂展示 PPTX。Phase 2B 进一步建立 State-Evidence Kernel 与 shadow canonical presentation compiler；Phase 2C 完成 disabled-by-default 的内部技术验证。目前生产 HTML/PPTX/ZIP 仍使用 legacy LessonBlueprint 路径，真实教学验证尚未开始。
...
1. Converge the white paper, architecture, README, roadmap, portfolio copy, and Agent Skill.
2. Generate and teacher-review three real Chinese micro-lessons: greetings/polite address; numbers/time/dates; restaurant ordering or shopping.
3. Select product work from pilot evidence: component gaps, teacher notes, self-study guidance, projection layout, editing workflow, or browser regression coverage.
4. Expand v2 modes and run a reversible cutover experiment only after real-lesson evidence.
5. Continue providers, themes, streaming progress, project history, and LMS exports according to demonstrated teacher value.

Browser automation is a supporting task and should not delay the first real-lesson pilot. Client-side accepted answers support offline practice; the product does not claim exam security or anti-cheating capability. See [roadmap.md](roadmap.md).
